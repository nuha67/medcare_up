...
export default function AutomatedBillingDesk() {
  ...
  const fetchBills = async (token: string) => {
    try {
      const response = await axios.get("https://34-229-165-55.nip.io/admin/billing/live-desk", { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handlePay = async (billId: number) => {
    ...
    try {
      await axios.post(`https://34-229-165-55.nip.io/bills/${billId}/pay`, {}, {
