...
export default function EmployeeManagementPage() {
  ...
  const fetchEmployees = async (token: string) => {
    try {
      const response = await axios.get("https://34-229-165-55.nip.io/admin/users/", {
  ...
  const handleRegisterEmployee = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.role === "Doctor") {
        endpoint = "https://34-229-165-55.nip.io/admin/doctors/";
        payload = {
        ...
        endpoint = "https://34-229-165-55.nip.io/admin/lab-techs/";
        payload = {
          full_name: `${formData.first_name} ${formData.last_name}`,
          dob: formData.dob,
          gender: formData.gender,
  ...
  const handleDeactivate = async (employeeId: string, employeeName: string) => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      await axios.put(`https://34-229-165-55.nip.io/admin/users/${employeeId}/deactivate`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
