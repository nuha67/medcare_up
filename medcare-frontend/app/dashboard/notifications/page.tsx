...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async (token: string) => {
    try {
      ...
      const notifRes = await axios.get("https://34-229-165-55.nip.io/patients/me/notifications", {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleMarkAsRead = async (notifId: number, isRead: boolean) => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      await axios.put(`https://34-229-165-55.nip.io/notifications/${notifId}/read`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
