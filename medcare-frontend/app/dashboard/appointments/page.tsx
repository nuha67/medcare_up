...
export default function UpcomingAppointmentsPage() {
  ...
  const fetchUpcomingAppointments = async (token: string) => {
    try {
      const response = await axios.get("https://34-229-165-55.nip.io/appointments/me/upcoming", {
  ...
  const handleCancelAppointment = async (appointmentId: number, refNumber: string) => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      await axios.put(`https://34-229-165-55.nip.io/appointments/${appointmentId}/cancel`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
