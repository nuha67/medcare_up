...
export default function LabTechDashboard() {
  ...
  const fetchLabDashboard = async (token: string, showLoader = true) => {
    ...
    try {
      const response = await axios.get("https://34-229-165-55.nip.io/lab-tech/dashboard", {
  ...
  const handleCollectSample = async (testId: number) => {
    try {
      ...
      await axios.put(`https://34-229-165-55.nip.io/lab-tests/${testId}/collect`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      if (!token) return; // 🟢 මේ පේළිය එකතු කරන්න (TS Error එක මකා දැමීමට)
      const response = await axios.post(`https://34-229-165-55.nip.io/lab-tests/${selectedTestId}/upload`, formData, {
        headers: { 
