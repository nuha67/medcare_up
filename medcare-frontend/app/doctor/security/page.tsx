...
export default function DoctorSecurityPage() {
  ...
  const fetchAllSecurityData = async (token: string) => {
    ...
    try {
      const [profileRes, sessionsRes] = await Promise.all([
        axios.get("https://34-229-165-55.nip.io/doctor/profile", config),
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      await axios.put("https://34-229-165-55.nip.io/doctors/me/profile/editable", {
  ...
  const handleGenerateQr = async () => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      const response = await axios.post("https://34-229-165-55.nip.io/auth/mfa/setup-app", {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleVerifyAppMfa = async () => {
    ...
    try {
      const token = localStorage.getItem("medcare_token");
      await axios.post("https://34-229-165-55.nip.io/auth/mfa/verify-app", { app_code: appVerifyCode }, { 
        headers: { Authorization: `Bearer ${token}` } 
      });
